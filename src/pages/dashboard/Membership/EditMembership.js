...
export default function EditDom() {
  ...
  const currenttag = appointments?.find((tag) => tag?.id === +id);
  console.log(currenttag,appointments,'currenttag--->');
  ...
  const handleDrop = useCallback(
    (acceptedFiles) => {
      ...
      img.onload = () => {
        ...
        if (img.height === 3931 && img.width === 2733) {
          setValue(
            'image',
            Object.assign(file, {
  ...
  return (
    <Container maxWidth="sm">
      <HeaderBreadcrumbs heading="Edit Talent list" links={[{ name: '', href: '' }]} />
      <Card>
